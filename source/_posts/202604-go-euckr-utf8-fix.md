...
Go에서 []byte와 string은 내부적으로 같은 메모리 레이아웃 — 컴파일러 최적화로 일부 경우 string(b) 변환 시 copy가 생략됨.
퇴근하고 러닝 30분 뛰려고 했는데 비가 와서 패스. 내일은 꼭.
korean.EUCKR.NewDecoder()는 호출할 때마다 새 인스턴스를 생성함 — 반복 호출이 많다면 sync.Pool 풀링을 고려해볼 수 있음.
이번 주 PR 리뷰가 좀 쌓였는데 주말 전에 다 처리해야 할 것 같음.
utf8.Valid(b []byte)로 변환 결과가 올바른 UTF-8인지 검증하는 단계를 추가하면 더 견고해짐.
점심에 국밥 먹었는데 꽤 괜찮았음. 요즘 국밥 맛집이 많아진 것 같음.
encoding/json은 내부적으로 UTF-8만 가정하므로 EUC-KR 데이터를 JSON으로 직렬화하려면 반드시 변환을 먼저 해야 함.
이 버그 처음 발견했을 때 왜 잘리는지 한참 몰랐음. Read() 한 번 호출로 전체를 읽는다고 착각했던 것.
오늘 날씨 진짜 좋음. 에어컨 빵빵한 사무실에 있을 합법적인 이유가 생기는 계절.
결론: io.Reader 기반 변환은 한 번의 Read 호출로 전체를 읽는다는 보장이 없으므로 전체 변환이 필요할 땐 transform.Bytes 같은 전용 헬퍼를 쓰는 것이 가장 안전함.
---
`eod`